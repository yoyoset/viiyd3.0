---
title: "寂静王萨雷克展示"
hero_title: "寂静王"
title_accent: "寂静王"
file_no: "034"
tagline: "六千万年的沉默——为夺回失去的一切而打破。萨雷克立于永恒之舟，高斯绿光与黄金交映。"
piece_count: "萨雷克 + 永恒之舟 + 2座石碑 + 4具卫士"
date: 2026-05-23T16:00:00+08:00
summary: "涅克隆王朝的最高统治者。虚空黑装甲与全身蔓延的幽绿高斯OSL光效，永恒之舟震慑全场。"
share_caption: "一尊模型，60 个小时——目前接过最长的单件，每个小时都花在刀刃上。

寂静王的虚空黑装甲得在整艘永恒之舟蔓延的幽绿高斯 OSL 下，还真的看起来是黑色——黑色稍微泛灰一点，光效的对比就全垮了。金色纹饰要显出暖调又不能抢了光效的风头，门胡巨石也得画出比这艘船本身更古老的年代感。

如果您手上有这种量级的项目，60 小时这种数字我宁愿老实报，也不愿意往少了说。"
tags: ["战锤 40,000", "涅克隆", "寂静王", "核心展品", "代工展示"]
cover: "https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_01.webp"
layout: "project"
photos: 14
optimized: true
tier: "Legend"
time_log: "60h 00m"
model_count: 1
paints:
  - name: "Abaddon Black"
    role: "BASE"
    hex: "#231F20"
    link: "https://www.warhammer.com/en-GB/shop/Base-Abaddon-Black-2019"
  - name: "Leadbelcher"
    role: "METALLIC"
    hex: "#7B8B8E"
    link: "https://www.warhammer.com/en-GB/shop/Base-Leadbelcher-2019"
  - name: "Retributor Armour"
    role: "GOLD"
    hex: "#DAA520"
    link: "https://www.warhammer.com/en-GB/shop/Base-Retributor-Armour-2019"
  - name: "Warpstone Glow"
    role: "OSL BASE"
    hex: "#2A6F3D"
    link: "https://www.warhammer.com/en-GB/shop/Layer-Warpstone-Glow-2019"
  - name: "Moot Green"
    role: "OSL HIGHLIGHT"
    hex: "#4CBB17"
    link: "https://www.warhammer.com/en-GB/shop/Layer-Moot-Green-2019"
  - name: "Sotek Green"
    role: "MENHIR STONE"
    hex: "#2E7D8C"
    link: "https://www.warhammer.com/en-GB/shop/Layer-Sotek-Green-2019"
  - name: "Nuln Oil"
    role: "SHADE"
    hex: "#2B2B2B"
    link: "https://www.warhammer.com/en-GB/shop/Shade-Nuln-Oil-2019"
description: "涅克隆寂静王萨雷克代工展示。虚空黑舟体配合高斯绿全域OSL，黄金萨雷克本体与青石方尖碑形成强烈视觉对比。共14张展示照。"
system: "40k"
delivered: "2026.05"
---

<div class="image-grid">
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_01.webp" title="寂静王 - 王座正面" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_02.webp" title="永恒之舟 - 左侧全景" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_03.webp" title="永恒之舟 - 右侧全景" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_04.webp" title="萨雷克本体 - 3/4 视角" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_05.webp" title="永恒之舟 - 背面结构" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_06.webp" title="高斯王座 - 能量细节" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_07.webp" title="萨雷克 - 黄金甲胄特写" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_08.webp" title="舟上守卫构体" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_09.webp" title="月牙战镰 - 刃面细节" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_10.webp" title="门希尔方尖碑 - 正面" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_11.webp" title="门希尔方尖碑 - 侧面轮廓" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_12.webp" title="OSL 光效 - 面板漫射研究" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_13.webp" title="地台细节 - 灰烬战场" >}}
{{< lightbox src="https://photo.viiyd.com/2026/05/silent-king/viiyd20260523silent-king_14.webp" title="寂静王 - 完整构图" >}}
</div>

<div class="space-y-4 my-4">

<div class="bg-gradient-to-r from-neutral-800 to-transparent border-l-4 border-green-500 p-6 rounded-r-lg">
    <h3 class="text-xl font-bold text-white mb-2">⚡ 最高意志：王朝的重生</h3>
    <p class="text-gray-300 leading-relaxed">
        萨雷克，最后的寂静王，带着一个目的从流放中归来——重新统一支离破碎的涅克隆王朝，直面混沌的威胁。这件代工要求一件能够统治全场的核心展品：虚空黑舟体吞噬一切光线，唯有高斯能量从每一根导管与线圈中喷涌而出，划破沉寂。门希尔方尖碑以古朴青石独立其外，裂纹与岁月共存，提醒着所有人：这个文明用肉身换取了永恒。
    </p>
</div>

<div class="grid md:grid-cols-2 gap-4">

<div class="bg-neutral-800/30 p-5 rounded border border-green-500/20 hover:border-green-500/40 transition-colors group">
    <div class="flex items-center justify-between mb-4 border-b border-white/5 pb-2">
        <h4 class="font-bold text-green-400 flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
            颜色协议
        </h4>
        <span class="text-[10px] uppercase text-gray-500">炼金数据</span>
    </div>
    <ul class="space-y-3 text-sm text-gray-300">
        <li class="flex justify-between">
            <span>⬛ 舟体面板</span>
            <span class="font-mono text-green-200">阿巴顿黑 + 深礁蓝</span>
        </li>
        <li class="flex justify-between">
            <span>✨ 高斯OSL</span>
            <span class="font-mono text-green-200">翘曲石绿 → 莫特绿</span>
        </li>
        <li class="flex justify-between">
            <span>🟡 萨雷克黄金</span>
            <span class="font-mono text-green-200">利贝托金甲</span>
        </li>
        <li class="flex justify-between">
            <span>🪨 方尖碑石质</span>
            <span class="font-mono text-green-200">索泰克绿 OSL 融合</span>
        </li>
    </ul>
</div>

<div class="bg-neutral-800/30 p-5 rounded border border-emerald-500/20 hover:border-emerald-500/40 transition-colors">
    <div class="flex items-center justify-between mb-4 border-b border-white/5 pb-2">
        <h4 class="font-bold text-emerald-400 flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            涂装协议
        </h4>
        <span class="text-[10px] uppercase text-gray-500">工艺亮点</span>
    </div>
    <p class="text-sm text-gray-400 mb-2">核心技术方案：</p>
    <ul class="space-y-2 text-sm text-gray-300 list-disc list-inside marker:text-emerald-500">
        <li><strong>全域OSL渗透:</strong> 高斯绿光向所有相邻表面漫射——面板、线圈和地台全部受光。</li>
        <li><strong>虚空黑梯度:</strong> 舟体面板从纯黑向边缘深蓝过渡，在保持阴影深度的同时增加层次感。</li>
        <li><strong>石质纹理对比:</strong> 方尖碑采用开裂青绿釉料区分古老石材与冰冷金属。</li>
    </ul>
</div>

</div>

</div>
