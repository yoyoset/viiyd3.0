...
- **「两单」口径**：不存在接单上限，是早期保质量的话术 → 全站删除（946c078）。档期块改「接单中」，不再写日期与名额；`data/schedule.yaml` 删除；关于页第 4 格改为「涉及系统」（按分类实时计算）。
- **关于页设备栏**：「设备齐全即可」→ 整块删除，原位置改为「出品」栏（d776678）：涂装全部手涂 / 实物实拍 / 颜料只写品牌（Citadel · AK · Vallejo · 国产）/ 网站 Claude Opus 5.5。
- **颜料清单**：实际混用 GW / AK / Vallejo / 国产，给不出逐单清单 → 新增 `palette:` 字段，标题「配色 · 取自实拍照片」，色值用 `scripts/extract_palette.js` 从实拍图提取，只写部位不写品牌（b6dab24、2da9683）。兽人页已用；用户给的唯一事实「GW 黄色系叠红色渍洗」写进 summary。
- **署名**：「艺术来自我们自己，技术来自 Opus 5.5」→ 页脚全站一行小字 + 关于页「出品」栏收尾一句（d776678）。文案用「VIIYD」而非「我们」，与「一个人，一张工作台」的定位不冲突。

## 仍待定

- **旧作品的 `paints:` 清单**：58 篇都是清一色 Citadel 型号，与「实际混用多品牌」不符，很可能是 3.0 时期生成的。是否逐步换成 `palette:`（可批量从 R2 图提取），等用户决定。
