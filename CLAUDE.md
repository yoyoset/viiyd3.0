...
- **署名**：页脚全站一行「艺术 · VIIYD 手涂 / 技术 · Claude Opus 5.5」，关于页「出品」栏收尾一句。
  只放页脚和关于页，不上首屏 —— 访客买的是手涂，AI 字样抢在作品前面会让人怀疑作品图是生成的。

## 定价与转化

- **没有接单名额 / 档期上限。** 「每次两单」「每季两个展示位」是早期话术，2026-09-26 已全站删除，不要再写回来。
...
- **2026-09 起，规格 §12「英文站不出 EIU / 询价改 Instagram」已被用户推翻**：中英文定价
  模型完全一样，没有理由只让中文客户看到报价。现在两边功能对等——首页计价卡、服务页
  EIU 数字、作品页「同款询价」按钮，中英文都出，CTA 统一走 `data-wechat-open`
  （`commission-modal.html` 已按语言分流默认态，英文访客点了自动落表单，不会看到二维码）。
  Instagram/Facebook 链接只保留在联系渠道类的位置（首页留白区、hook 区），
  不再是询价的替代品。
